...
function getPreferredStoreUrl(game: Game) {
    if (typeof window === "undefined") {
        return game.appStore ?? game.playStore;
    }
    ...
    if (isIos && game.appStore) {
        return game.appStore;
    }

    if (isAndroid && game.playStore) {
        return game.playStore;
    }

    return game.appStore ?? game.playStore;
}

function openPreferredStore(game: Game) {
    const storeUrl = getPreferredStoreUrl(game);
...
function StoreLinks({ game }: { game: Game }) {
    if (!game.appStore && !game.playStore) {
        return null;
    }
    ...
    return (
        <>
            {/* 어두운 배경 레이어 */}
            <div
                className="store-links-dim absolute inset-0 hidden bg-black opacity-0 transition duration-300 pointer-events-none sm:block group-hover:opacity-60"/>

            {/* 아이콘 컨테이너 */}
            <div
                className="store-links-overlay absolute inset-0 z-20 hidden items-center justify-center gap-4 opacity-0 transition duration-300 pointer-events-none sm:flex group-hover:opacity-100 group-hover:pointer-events-auto">
                {/* iOS 버튼 */}
                {game.appStore && (
                    <a
                        href={game.appStore}
                        ...
                        aria-label={`${game.title} App Store`}
                ...
                {game.playStore && (
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        aria-label={`${game.title} Play Store`}
                        onClick={(event) => event.stopPropagation()}
                        className="bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md hover:scale-110 transition"
                    >
                        <svg
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-10">
                        {games.map((game, i) => (
                            ...
                            >
                                ...
                                >
                                    {(game.appStore || game.playStore) && (
                                        <button
                                            type="button"
                                            aria-label={`${game.title} 스토어 페이지 열기`}
                                            className="absolute inset-0 z-10 cursor-pointer border-0 bg-transparent p-0"
                                            onClick={() => openPreferredStore(game)}
                                        />
                                    )}
                                ...
                                <div className="mt-5 min-h-12 transition duration-300 ease-out group-hover/card:-translate-y-1">
                                    ...
                                    {game.status && (
                                        <p className={game.hideTitle
                                            ? "mt-0 text-base sm:text-lg font-bold leading-tight text-white transition duration-300 group-hover/card:text-neutral-100"
                                            : "mt-1.5 text-xs sm:text-sm font-medium text-neutral-500 leading-tight transition duration-300 group-hover/card:text-neutral-300"
                                        }>{game.status}</p>
                                    )}
                                </div>
                            </div>
